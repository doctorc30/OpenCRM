...
BEGIN TRANSACTION
GO
PRINT N'Выполняется удаление [dbo].[FK_Events_AspNetUsers]...';


GO
ALTER TABLE [dbo].[Events] DROP CONSTRAINT [FK_Events_AspNetUsers];
...
GO
PRINT N'Выполняется создание [dbo].[FK_Events_AspNetUsers]...';


GO
ALTER TABLE [dbo].[Events] WITH NOCHECK
    ADD CONSTRAINT [FK_Events_AspNetUsers] FOREIGN KEY ([UserId]) REFERENCES [dbo].[AspNetUsers] ([Id]) ON DELETE CASCADE;
