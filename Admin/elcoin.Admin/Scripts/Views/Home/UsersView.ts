...
class UsersView {
    ...
    constructor(drawtreeCallBack: (container: string, data: any, options: any) => void) {
        this.users = [];
    ...
    fillUsers() {
        var table = $('#firstLineTable').DataTable(<any>{
            pageLength: 10,
            ajax: {
...
// ReSharper restore SuspiciousThisUsage
    ...
    moveUser() {
        var moveToUserName: string = $("#targetName").val();
        $.ajax({
            type: "POST",
            data: {
