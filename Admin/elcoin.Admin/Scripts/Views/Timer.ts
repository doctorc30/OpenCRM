class Timer {
    ...
    getTimer(string) {
        ...
        if (dateT > date) {
            ...
            day = day.toString();
            var hour: any = /*Math.round((dateT.getTime() - date.getTime()) / (1000 * 60 * 60));//*/this.getMax(dateT.getHours(), date.getHours(), 24);
            if (parseInt(hour) < 10) {
                hour = "0" + hour;
            }
            hour = hour.toString();
            var min: any = /*Math.round((dateT.getTime() - date.getTime()) / (1000 * 60));//*/this.getMax(dateT.getMinutes(), date.getMinutes(), 60);
            if (parseInt(min) < 10) {
                min = "0" + min;
            }
            min = min.toString();
            var sec: any = /*Math.round((dateT.getTime() - date.getTime()) / (1000));//*/this.getMax(dateT.getSeconds(), date.getSeconds(), 60);
            if (parseInt(sec) < 10) {
                sec = "0" + sec;
            }
            sec = sec.toString();
